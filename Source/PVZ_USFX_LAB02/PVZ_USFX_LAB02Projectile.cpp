...
void APVZ_USFX_LAB02Projectile::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if ((OtherActor != nullptr) && (OtherActor != this) && (OtherComp != nullptr))
	{
		...
		if (OtherActor->ActorHasTag("Enemy"))
		{
			...
			float newEnemyHealth = OtherActor->TakeDamage(DamageGenerates, FDamageEvent(), nullptr, this);
			GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("SpawnAfter: %f"), newEnemyHealth));
